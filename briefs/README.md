...
| 19 | `heatmap-refresh-theme.md` | gap-fix, v1.22.0 -- **EXECUTED 2026-09-17** (user-reported; planner turn-limited -> lead re-emitted the plan from the brief, coder landed all tasks first pass, reviewer APPROVED 7/7 judgment calls zero blockers, qa=lead; 558/558 + torture ok + 1 reversion proof; release pending) | S | The grid kernel (createHeatmap) was the only kernel without `refreshTheme()` -- and its mount() resolved the six theme specs IN PLACE, destroying the `--var` tokens, so the fix required a construction-time `_themeSpecs` capture + a shared `_resolveThemeColors()` + a cold `computeColors` re-run (cell colors are precomputed at extract). Closed a types-vs-impl mismatch (Charts.d.ts already declared the method on `Chart`) and fixed remount-after-unmount double-resolution in passing. |
...
candidate (`error-bars.md`, release pending); next up is item 3: axis titles
+ tick-format callback; then chart chrome; linked-chart helpers; a11y live
