...
Current state: **v1.23.0 SHIPPED** -- axis titles + tick-format callbacks +
the axis-kernel theme fix, brief #20 executed + released 2026-09-21 (572/572 +
torture A28, release commit 9d1ed57, published + npm-view-verified +
card-synced same day; Cut 0 fixed a latent bug live since v1.0.0: axis chrome
never recolored on refreshTheme). Polar/radar kernels AUDITED CLEAN 2026-09-21
for the same staleness class (all themed refs are draw-time reads, no scene
bindings -- no brief needed). Next up: **demo session 3**
(`demo-refresh-3.md`, written 2026-09-21 -- v1.23.0 showcase + the
--c-emerald/--c-rose token repair); library successor = queue item 4 chart
chrome (brief #21, unwritten). Prior: **v1.22.0** -- heatmap refreshTheme
parity, brief #19 executed 2026-09-17 (grid kernel `refreshTheme()`, 558/558
+ torture, release commit ef97317, published + card-synced 2026-09-17). Demo
sessions 8a + 8b both EXECUTED + verified 2026-09-17 and committed by the user
(60f5c86, 04f6036) -- demo shows v1.13->v1.21 end to end incl. the 3D
relief headline; the `geometry.heightfield` swap was DECIDED AGAINST (see 8a). Prior: **v1.21.0** -- error bars /
confidence bands, brief #18 executed 2026-09-08, released + published +
npm-view-verified + card-synced 2026-09-17 (per-series `errorBars: {lo,hi}|{value}` whiskers and/or ribbon on
...
`setBrush` null-gate fix; two review finds fixed -- brushMoved latch +
onBrushAbort; 543/543 + A26 + 4 reversion proofs, reviewer REJECTED-then-
fixed). Published + npm-view-verified 2026-09-08 (latest=1.20.0); catalog
card synced. Prior: **v1.19.0** -- candlestick/OHLC chart, brief
...
5 reversion proofs). Prior: **v1.18.0** (npm-view
...
field+contour panel added same day, demo-only). Earlier: v1.16.0 (injected
...
| ~~17~~ | ~~`brush-refinements.md`~~ | feature, v1.20.0 -- **SHIPPED 2026-09-08** (published + npm-view-verified, card synced; full pipeline: planner -> coder x3 -> reviewer REJECTED-then-fixed -> qa; 543/543 tests incl. 29 new + HB2 re-pin, torture ok + new A26 gate, 4 measured reversion proofs; ALL THREE cuts landed, no v1.21.0 split) | M | Brush v2: configurable brush modifier (shift hardcoded at Charts.js:7472/:7629); brush IDs across all visible series (additive `idsBySeries`, primary `ids` unchanged); horizontal-bar band multi-select (toggle-click, non-contiguous `bands`, hull bandMin/bandMax, pooled run-baked overlay); PLUS a mandatory fail-closed fix -- vertical `brushFacade.set` has no `== null` gate (:6810-6816, `setBrush({xMin:null})` -> silent 0; the horizontal branch was fixed in v1.9.0, vertical never was). Cuts 1+2+fix alone are a releasable v1.20.0; cut 3 may split to v1.21.0. |
| 18 | `error-bars.md` | feature, v1.21.0 -- **EXECUTED 2026-09-08** (queue item 2 of 7; planner stalled -> lead re-emitted the plan, coder landed T1-T7 + .d.ts before its turn limit, lead added torture A27, reviewer APPROVED zero blockers + 2 nits fixed, qa=lead; 552/552 + torture A27 + three reversion proofs; release pending) | M | First statistical-series cut, the ROADMAP's cheapest: per-point `lo`/`hi` accessors (absolute + symmetric `value` sugar) projected through the y-scale on the annotation cold-resolve/hot-project 0-B/frame idiom -- whiskers (error bars) and/or a filled ribbon (confidence band), on line/area/scatter. NO new kernel; DECORATES existing continuous-x renderers, candle's per-point-column precedent (os/hs/ls/cs -> pos/phs/pls/pcs). Per-series config (multi-series), null/NaN self-skips per point (never anchors at 0 -- the `+null===0` trap), construction-throws on junk config. OUT (named): bar error bars (grouped/stacked offset math), decimated whiskers, horizontal, box plot/stacked area (the NEXT cuts). Open planner decisions: per-series vs chart-level config placement, whether the band ships with the whiskers or fast-follows. |
| 19 | `heatmap-refresh-theme.md` | gap-fix, v1.22.0 -- **EXECUTED 2026-09-17** (user-reported; planner turn-limited -> lead re-emitted the plan from the brief, coder landed all tasks first pass, reviewer APPROVED 7/7 judgment calls zero blockers, qa=lead; 558/558 + torture ok + 1 reversion proof; **SHIPPED 2026-09-17**, published + npm-view-verified, card synced) | S | The grid kernel (createHeatmap) was the only kernel without `refreshTheme()` -- and its mount() resolved the six theme specs IN PLACE, destroying the `--var` tokens, so the fix required a construction-time `_themeSpecs` capture + a shared `_resolveThemeColors()` + a cold `computeColors` re-run (cell colors are precomputed at extract). Closed a types-vs-impl mismatch (Charts.d.ts already declared the method on `Chart`) and fixed remount-after-unmount double-resolution in passing. |
...
| 8a | `demo-refresh-1.md` | demo-only, no release -- **SESSION 1 of 2** (written + EXECUTED 2026-09-17, verified live, committed 60f5c86; heightfield swap DECIDED AGAINST 2026-09-17 -- inline banded builder stays, see the brief; the v1.6->v1.12 pass was DONE 2026-09-02) | M | Foundation + marquee: branding -> v1.21.0, importmap adds lite-depth@2.0.0, add the missing `createCandlestickChart` import, then three panels -- the headline **3D relief map** (field-raster grid extruded via @zakkster/lite-depth; DPR pass-through; BANDED ramp = contours-in-3D because depth flat-shades one material/node; inline geometry.custom builder against published depth@2.0.0, swaps to `geometry.heightfield` if the parallel LiteDepth brief ships -- NOT blocked on it), candlestick (v1.19.0), error bars (v1.21.0). delaunay stays 1.3.0 (relief reuses createFieldIndex). Charts.js/d.ts/test UNTOUCHED. |
| 20 | `axis-titles-tickformat.md` | feature, v1.23.0 -- **SHIPPED 2026-09-21** (executed + released same day: 572/572 + torture A28 + 3 reversion proofs, reviewer APPROVED zero blockers; release commit 9d1ed57, published + npm-view-verified, card synced; PLUS Cut 0, a discovered latent bug live since v1.0.0: axis-kernel refreshTheme never recolored its scene-node chrome -- fixed via a tracked axisThemeVersion signal; rotated-text SVG export fixed in the same cut; 3 qa-caught spec/impl fixes in the AS-EXECUTED block; polar/radar audited clean of the staleness class post-release) | S-M |
| 21 | `chart-chrome.md` | feature, v1.24.0 candidate -- **EXECUTED 2026-09-22** (queue item 4; 583/583 + torture A29 + 3 reversion proofs, reviewer APPROVED 6/6 zero blockers, coder zero deviations first pass; demo candlestick wears the chrome + branding v1.24.0 same session, verified live; release pending) | S-M | Chart-LEVEL chrome on the axis kernel riding the v1.23.0 machinery: `title`/`subtitle` stack canvas-centered at the top (conditional default-margin bumps 24/16px), `caption` right-aligns on the bottom-most line (14px bump, pushes the v1.23.0 xTitle up one line when both present). Fonts derive cold from config.font (title bold +4px / subtitle +1 / caption -1, no-px fallback fail-safe); colors ride labelColor + axisThemeVersion. Doors pre-signal incl. subtitle-requires-title. OUT (named): per-kernel chrome (pie/radar/heatmap), per-element {text,color,font} objects, wrapping. |
| 8c | `demo-refresh-3.md` | demo-only, no release -- **EXECUTED 2026-09-21** (verified live both themes, zero console; 572/572 unchanged; UNCOMMITTED -- rides the working tree with the #21 cycle, user commits) | S | v1.23.0 showcase + token repair: define the MISSING `--c-emerald`/`--c-rose` :root tokens (six charts render fallback grey today, and the demo copy documents the token as working); branding v1.21->v1.23; `yTitle`+`yTickFormat` units on the error-bars panel + epoch-ms date `xTickFormat` on a time-series panel; theme-toggle copy (axis chrome now genuinely recolors -- the latent bug this very demo never surfaced). Charts.js/d.ts/test UNTOUCHED. | Two cuts on the axis kernel: `xTickFormat`/`yTickFormat` user callback replacing `formatTickValue` at the ONE cold call site (Charts.js:1806; fail-closed on non-function + non-string return; band axes ignore it, decision recorded), and `xTitle`/`yTitle` pooled scene text nodes (yTitle via native lite-scene `rotation`; margin auto-bump ONLY when that side's margin is un-set; theme-refresh + exportSVG + destroy asserted). Secondary y-axis and chart chrome stay OUT. |
| 8b | `demo-refresh-2.md` | demo-only, no release -- **SESSION 2 of 2** (written + EXECUTED 2026-09-17, verified live, committed 04f6036) | M | Catch-up panels, independent of 8a after its branding lands: cluster outlines (v1.18.0, REUSES the existing two-cluster dataset :2646-2655, bumps delaunay 1.3.0->1.4.0), brush v2 (v1.20.0, brushModifier + idsBySeries + hbar band multi-select), overnight/holidays + early close (v1.13.0/v1.15.0, extend the time-series section), horizontal legend virtualization (v1.15.0, top/bottom, orientation-exclusive width/itemWidth). Charts.js/d.ts/test UNTOUCHED. |
...
2026-09-06).
~~#17 brush-refinements -> v1.20.0~~ SHIPPED 2026-09-08 (all three cuts
+ the fail-closed fix; published + npm-view-verified, card synced).
Remaining -- per the ROADMAP "Queue (2026-09-06, user-confirmed order)",
item 2 (error bars / confidence bands) is EXECUTED 2026-09-08 as the v1.21.0
candidate (`error-bars.md`, SHIPPED v1.21.0); item 3 (axis titles +
tick-format callback) now has brief #20 (`axis-titles-tickformat.md`,
written 2026-09-21) -- the NEXT session; then chart chrome; linked-chart
helpers; a11y live region; data labels -- each later item still needs a
brief before its cut.
