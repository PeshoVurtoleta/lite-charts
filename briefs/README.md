...
Current state: **v1.22.0 SHIPPED** -- heatmap refreshTheme parity, brief #19
executed 2026-09-17 (grid kernel `refreshTheme()`, 558/558 + torture, release
commit ef97317, published + npm-view-verified + card-synced 2026-09-17). Demo
sessions 8a + 8b both EXECUTED + verified 2026-09-17 and committed by the user
(60f5c86, 04f6036) -- demo now shows v1.13->v1.21 end to end incl. the 3D
relief headline; the `geometry.heightfield` swap was DECIDED AGAINST (see 8a).
Next up: **brief #20** (`axis-titles-tickformat.md`, queue item 3, v1.23.0
candidate -- written 2026-09-21). Prior: **v1.21.0** -- error bars /
...
| 20 | `axis-titles-tickformat.md` | feature, v1.23.0 candidate -- **EXECUTED 2026-09-21** (572/572 + torture A28 + 3 reversion proofs, reviewer APPROVED zero blockers; PLUS Cut 0, a discovered latent bug: axis-kernel refreshTheme never recolored its scene-node chrome -- fixed via a tracked axisThemeVersion signal; 3 qa-caught spec/impl fixes recorded in the AS-EXECUTED block; release pending) | S-M | Two cuts on the axis kernel: `xTickFormat`/`yTickFormat` user callback replacing `formatTickValue` at the ONE cold call site (Charts.js:1806; fail-closed on non-function + non-string return; band axes ignore it, decision recorded), and `xTitle`/`yTitle` pooled scene text nodes (yTitle via native lite-scene `rotation`; margin auto-bump ONLY when that side's margin is un-set; theme-refresh + exportSVG + destroy asserted). Secondary y-axis and chart chrome stay OUT. |
