...
**Status:** v1.23.0 -- axis titles + tick-format callbacks (minor), plus an
axis-kernel theme fix. `xTickFormat` / `yTickFormat` callbacks format the
numeric/time tick labels; `xTitle` / `yTitle` render axis titles (the y
title rotated) with an automatic margin bump on unset sides. Fixed: the
axis kernel's `refreshTheme()` never actually recolored its spine, ticks,
labels, or gridlines (stale scene-binding cache) -- it does now; and
`exportSVG()` renders rotated text (pie/radar labels included) instead of
flattening it.
**572/572 tests pass** plus a torture/stress gate (`npm run torture`).
...
| v1.23.0+ (candidates) | In confirmed order (see ROADMAP): chart title/subtitle/caption in the layout system; linked-chart helpers; crosshair/tooltip ARIA; data labels on bars/points. Deferred with named triggers: volume pane, index-compact x, candlestick variants, low-GC transitions, secondary y-axis. |
