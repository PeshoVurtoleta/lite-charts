...
**Status:** v1.22.0 -- heatmap `refreshTheme()` (minor). The grid kernel
now has the same theme hook every other kernel had: `chart.refreshTheme()`
on a heatmap re-resolves the six theme-affected colors from their ORIGINAL
specs (CSS-var tokens survive mount), recomputes the precomputed per-cell
color arrays, and repaints. Cold path; the per-frame draw is untouched.
Also fixes remount-after-unmount, which re-resolved already-concrete
values instead of the original tokens.
**558/558 tests pass** plus a torture/stress gate (`npm run torture`).
...
`refreshTheme()`; earlier versions required a re-create.
...
| v1.22.0+ (candidates) | In confirmed order (see ROADMAP): axis titles + zero-alloc tick formatters; chart title/subtitle/caption in the layout system; linked-chart helpers; crosshair/tooltip ARIA; data labels on bars/points. Deferred with named triggers: volume pane, index-compact x, candlestick variants, low-GC transitions. |
